...
- **IIV POLICY**: Every PK parameter in EVERY template (1-cmt, 2-cmt, 3-cmt) starts WITH IIV (EXP(ETA(n))) and a corresponding $OMEGA row (0.04 for peripheral params). This is the DEFAULT. IIV on peripheral parameters (Q, V2, Q3, V3) should only be fixed/removed when diagnostics show RSE > 50% (relative standard error of the IIV variance). High shrinkage (>30%) is a WARNING signal that the estimate may be unreliable and should be reported, but it is NOT by itself a reason to fix — only %RSE > 50% triggers consideration of fixing. Fixing may also be forced when convergence fails. When a model converges successfully with fixed IIV, the next run MUST attempt to UNFIX one peripheral IIV at a time.
